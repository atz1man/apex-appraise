...
export default function AutoAppraisal() {
  ...
  const onGenerate = async () => {
    if (phase === 'loading' || notes.trim().length < 10) return;
    setError(null);
    setPhase('loading');
    try {
      const res = await extract.mutateAsync({ notes, buildPerSqft: buildRate });
      setRun({
        extraction: res.extraction,
        buildPerSqft: buildRate,
        indicative: res.indicative,
        assetLabel: res.extraction.assetType,
        sourceNote: 'Generated by AI from your document text.',
      });
      setChat([]);
      setPhase('result');
    } catch (e) {
          ...
          {engine === 'ai' ? (
            <>
              ...
              <div className="mt-3.5 flex items-center gap-3">
                ...
                <button
                  onClick={onGenerate}
                  disabled={phase === 'loading' || notes.trim().length < 10}
