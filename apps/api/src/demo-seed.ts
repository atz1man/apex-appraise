/**
 * Demo-org seed mirroring the design-reference prototypes so the app is never
 * empty in sales demos: 11 pipeline deals, the Bournemouth trade-counter
 * appraisal, sales/lettings units, cost packages + contractors, photo log,
 * data room, investors, comparables, scenarios and benchmark aggregate.
 *
 * Called by the seed script (prisma/seed.ts, after a full wipe) and by the
 * nightly demo reset (POST /admin/reset-demo, after an org-scoped cascade
